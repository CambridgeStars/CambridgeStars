...
subroutine funcs1 ( jk, ji, var, dvar, fn1, eosout, abundout, px )
   ...
   if (usemenc .and. m<maximum_match_mass .and. ip_mesh_size > 0) then
      if (m > th(VAR_MASS,ip_mesh_size)) lom=lom_enc
   end if
   ...
   s2  = grada*scp*t
   if (dg_ledoux > 0.0d0) then
      wc1 = 0.5d0*s2*(calp*calp*hp/(9.0d0*xhi))**2
      wc2 = 546.75d0*wc1*dg_ledoux + 73.d0
      wc3 = cbrt(wc2 + sqrt(wc2**2 + 23.d0**3))
      wc4 = wc3 - 23.d0/wc3 - 2.0d0
   ...
   if ( jk == 1 ) then     ! Surface boundary conditions
      ...
      LoLedd = max(min(LoLedd,0.99_dbl),0.0_dbl)
      ...
      rht = (0.755d0*absl**0.47d0 + 0.05d0*absl**0.8d0)/m**0.31d0
      rad = r/rht
      w2 = r*absl/m
      ...
      if ( mutate .or. (jmod == 0 .and. gentle_start_massloss)) zep(Jstar) = 0.0d0
      ...
      !     Woosley 2000, eq 50)
      wowcrit = min(abs(omega)/sqrt(cg*mor3*(1.0d0-LoLedd)), 1.0_dbl)
      ...
      mdot_cmi = 0.0d0
      if ( (jmod /= 0 .or. .not. gentle_start_massloss) .and. cmi /= 0.0d0 ) then
         ...
         if (jmod < 10 .and. gentle_start_massloss) mdot_cmi = mdot_cmi * jmod / 10.d0
      end if
      ...
      if ( Jstar == 2 ) then
         ...
         do j = 1, 2
            ...
            brp(j) = brp(j)*(1.0d0 - cmtwl * wowcritp(j))
            brp(j) = 1.0d0 - brp(j)
         end do
