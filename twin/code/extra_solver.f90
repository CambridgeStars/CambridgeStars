...
subroutine checks
   ...
   ! Avoid unphysical values for mass, radius and temperature.
   ! These can only cause things to break.
   do ik = 1, kh-1
      do jstar = 1, isb
         ...
         ij = idx_for_star(VAR_LNT, jstar)
         if ( h(ij, ik) + dh(ij, ik) < 6.9d0 ) dh(ij, ik) = 0.0d0
      end do
   end do
   ...
   do jstar = 1, isb
      ij = idx_for_star(VAR_LUM, jstar)
      if ( h(ij, 1) + dh(ij, 1) < 0.0d0 ) dh(ij, 1) = 0.0d0
   end do
  ...
  !  and it is actually clearing DH that helps.
   ...
   if (allow_underrelaxation) then
     ! Try reducing the convective mixing
      ...
      call fudged_solver ( iter, ig, kt5, jo, mixing_fudge, fh, dfh, verbosity)
     ...
     ! Try smoothing out the thermal energy contribution to the luminosity equation
      ...
      call fudged_solver ( iter, ig, kt5, jo, luminosity_fudge, fh, dfh, verbosity)
   end if

   if (allow_mdotrelaxation) then
      if (jo /= 0 .and. verbosity>0) write (1, *) 'attempting to underrelax cmi term'
      call fudged_solver ( iter, ig, kt5, jo, mdot_smooth, fh, dfh, verbosity)
   end if
   ...
   if (allow_avmurelaxation .and. .not. use_previous_mu) then
      ...
      call fudged_solver ( iter, ig, kt5, jo, avmu_smooth, fh, dfh, verbosity)
   end if
...
!   COMMON DH(:,:)         - Corrections for the current timestep
! ------------------------------------------------------------------------------
subroutine fudged_solver ( iter, ig, kt5, jo, factor, fh, dfh, verbosity )
   use real_kind
   use mesh
   use mesh_enc
   use settings
   ...
   implicit none
   integer, intent(in) :: iter, kt5, ig(130), verbosity
